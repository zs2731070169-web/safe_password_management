...
| `version` | 单调递增版本号，服务端拒绝旧 / 乱序请求覆盖新快照（防回退） | ✅ |
| `checksum` | `ciphertext` 的哈希（如 SHA-256），校验传输 / 存储未损坏 | ✅ |
| `force`（可选） | 显式「用本机数据覆盖云端」开关，默认 `false`；`true` 时绕过版本序检查、无条件覆盖并重建单调基线（换机兜底，须前端二次确认，绝不在自动备份链路默认携带） | ✅ |

要点：

- **快照范围（决策点 B）**：`ciphertext` 解密后的整库 JSON **同时包含活跃条目与回收站条目**（回收站项保留 `deletedAt`），使「30 天可恢复」窗口在换机后延续。故软删除 / 从回收站恢复也改变快照、需触发上传。
- **覆盖式 + 防回退（单设备无同步定位）**：用单调 `version` 拒绝旧 / 乱序请求覆盖新快照，但**不做合并**。因「不涉及多设备同步」，`version ≤ 云端` 的 409 是**客户端内部并发信号**（乱序 / 重试旧请求 → 静默丢弃），**而非要用户处理的同步冲突**；换机未先拉取的极端态由恢复流程（先 `GET /backup`）或用户显式 `force=true` 覆盖解决，不让单设备用户背负「拉取—合并—重试」的同步心智。
