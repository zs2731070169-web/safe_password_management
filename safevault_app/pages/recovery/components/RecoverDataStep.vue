<script setup>
/**
 * RecoverDataStep —— 重置密码后取回数据：输入恢复码恢复 / 无恢复码时放弃旧数据重建（uni-app 版，可复用）
 *
 ...
 *
 * 复用场景：①重置流程步骤二（reset.vue）；②设置页「数据待恢复」再入口（recover.vue）。
...
async function handleRecover() {
  ...
  try {
    ...
    if (ok) {
      ...
      toastError('恢复码不正确，请检查后重试')
    }
  } catch (err) {
    if (err?.name === 'AbortError') return
    toastError(err?.message || '恢复失败，请重试')
  } finally {
...
async function doRebuild() {
  ...
  try {
    ...
    if (newCode) {
      ...
      toastError('重建失败，请确认已登录后重试')
    }
  } catch (err) {
    if (err?.name === 'AbortError') return
    toastError(err?.message || '重建失败，请稍后重试')
  } finally {
  ...
  <view class="recover-step">
    ...
    <template v-else>
      ...
      <view class="recover-status">
        <view class="recover-status__badge">
          <AppIcon name="recovery-key" :width="22" :height="22" />
        </view>
        <text class="recover-status__title">输入恢复码找回数据</text>
        <text class="recover-status__desc">
          密码已重置。输入注册时保存的恢复码即可找回此前的全部数据；没有恢复码则旧数据无法恢复。
        </text>
      </view>

      <!-- 恢复码输入卡片 -->
      <view class="recover-card">
        <view class="recover-field">
          <text class="recover-field__label">恢复码</text>
          <input
      ...
      <button
        ...
        @click="handleRebuild"
      >
        <text>{{ rebuilding ? '重建中…' : '没有恢复码？放弃旧数据并重建' }}</text>
      </button>
    </template>
