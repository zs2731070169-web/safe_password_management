 ...
 *
 * —— 自源 views/about/PrivacyPolicyView.vue 迁移 ——
 *   - vue-router(router.back/replace) → navBack；
 *   - 标签 div/header/main/section/h1/h2/p/ul/li/span → view/text；
 *   - 大段正文每段独立 <text>（App 端 view 直接子文本不渲染）；
...
<template>
  ...
  >
    ...
    <view class="privacy-header">
      ...
      <view class="privacy-header__placeholder" aria-hidden="true"></view>
    </view>

    <view class="privacy-page__main">
      <view v-for="(sec, i) in SECTIONS" :key="i" class="privacy-section">
        <text class="privacy-section__title">{{ sec.title }}</text>
        <!-- 每段正文独立 text：App 端 view 直接子文本不渲染，长文必须各自包 text -->
        <text
        ...
        <view v-if="sec.bullets" class="privacy-section__list">
          <text
            v-for="(b, bi) in sec.bullets"
            :key="`b-${bi}`"
