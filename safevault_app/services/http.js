 ...
 *
 * 按运行目标切换本值：
 *   - 真机调试（当前值）：电脑局域网 IP（手机须连同一 WiFi），且后端须以 `--host 0.0.0.0` 启动
 *     （默认只绑 127.0.0.1，局域网连不进来）；本机 WiFi IP 可用 `ipconfig getifaddr en0` 查询。
 *   - Android 模拟器：'http://10.0.2.2:8000'（映射到宿主机回环）；
 *   - iOS 模拟器 / 桌面：'http://127.0.0.1:8000'（与宿主共享网络）。
 * 生产 / 正式 APK 由构建期 VITE_API_BASE_URL 注入真实源站，优先级高于此默认值（见下方 BASE_URL）。
 */
const APP_DEV_API_BASE = import.meta.env.VITE_APP_DEV_API_BASE || 'http://127.0.0.1:8000'
