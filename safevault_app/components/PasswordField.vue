...
function onInput(event) {
  // uni input 的输入值在 event.detail.value；H5/浏览器为 event.target.value，两端兼容取值
...
<template>
  <view class="pw-field" :class="{ 'pw-field--error': error }">
    <view class="pw-field__label">{{ label }}</view>

    <view class="pw-field__control">
      <!-- uni input：密文用 :password 属性（非 type=password）；回车提交用 @confirm（非 @keydown.enter）；
           autocomplete/autocapitalize 等 Web 属性 uni 不支持，移除。 -->
      <input
    ...
    <view v-if="error && errorText" class="pw-field__error">{{ errorText }}</view>
  </view>
</template>
