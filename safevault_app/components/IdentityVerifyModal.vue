...
// ESC 监听 + 锁滚动是 H5 专属的 DOM 副作用：用两个小函数封装，App 端编译为空，
// 使下方 watch / onBeforeUnmount 的「状态复位 + cleanup」业务逻辑在两端都正常执行。
// #ifdef H5
function onKeydown(event) {
  if (event.key === 'Escape') onCancel()
}
function bindDomEffects(open) {
  if (open) {
...
function unbindDomEffects() {
  document.removeEventListener('keydown', onKeydown)
  document.body.style.overflow = ''
}
// #endif
// #ifndef H5
// App 端无 document：DOM 副作用空实现，保留调用点不变
function bindDomEffects() {}
function unbindDomEffects() {}
// #endif

// 打开/关闭：监听 ESC（H5）、锁定滚动（H5）、复位状态（两端）。
...
<!-- 不用 Vue 内置 <Transition>：uni-app app-vue 端 <Transition> 在真机上不可靠（离场卡住、
     甚至首次实例化即异常导致页面闪退）。改为元素常驻 + --open 类切换：关闭态用
     visibility/opacity/pointer-events 收起（不拦截点击），开启态恢复可见可点，淡入 + 面板缩放由
     CSS transition 承载（与 ConfirmSheet / SettingItem 的 option-sheet 同思路，App/H5 双端可靠）。 -->
<template>
      <view class="iv-modal" :class="{ 'iv-modal--open': modelValue }" @click.self="onCancel">
        <view
        ...
        >
          <text id="iv-modal-title" class="iv-modal__title">{{ title }}</text>
          <view v-if="hint" class="iv-modal__hint">
          ...
          <template v-if="mode === 'biometric'">
            <button
              ...
              @click="onBiometric"
            >
              <text class="iv-bio__ring">
                <AppIcon name="fingerprint" :width="34" :height="38" />
              </text>
              <text class="iv-bio__label">
                {{ verifying ? '正在验证指纹…' : '轻触使用指纹验证' }}
              </text>
            </button>

            <view v-if="errorMsg" class="iv-modal__error iv-modal__error--center">{{ errorMsg }}</view>

            <view class="iv-hr"></view>
          ...
          <!-- 主密码验证模式 -->
          <template v-else>
            <view class="iv-input" :class="{ 'iv-input--error': errorMsg }">
              <!-- uni input：密文用 :password 属性；回车提交用 @confirm；autocomplete 等移除 -->
              <input
            ...
            <view v-if="errorMsg" class="iv-modal__error">{{ errorMsg }}</view>
...
.iv-modal {
  ...
  &__panel {
    ...
    text-align: center;
    transform: scale(0.95); // 关闭态轻微缩小，开启态弹回
