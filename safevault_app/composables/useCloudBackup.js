...
export function useCloudBackup() {
  ...
  async function runAuto() {
    ...
    try {
      ...
      toastError(err?.message || '云备份失败，请稍后重试')
    }
  }
  ...
  async function backupNow({ force = false, silent = false } = {}) {
    ...
    try {
      ...
      if (res.status === 'ok' && !silent) toastSuccess('已备份到云端')
  ...
  watch(
    ...
    () => {
      // 程序权威覆盖（云端恢复 / 登录水合的 replaceFromSnapshot）引发的整库替换：消费一次性标志并
      // 跳过本次回写，避免「刚从云端下载又原样 PUT 回去」的多余请求。仅抑制这一拍，不影响后续用户改动。
      if (vault.replacingFromSnapshot) {
        vault.replacingFromSnapshot = false
        return
      }
      schedule()
    },
