...
export function useCloudAccount() {
  ...
  async function sendCode(email) {
    try {
      ...
      if (ok) toastSuccess('验证码已发送，请查收邮箱')
      return ok
    } catch (err) {
      if (err?.name === 'AbortError') return false
      toastError(err?.message || '验证码发送失败，请重试')
      return false
    }
  }
  ...
  async function register(payload) {
    try {
      ...
      if (ok) toastSuccess('云账户已创建')
      return ok
    } catch (err) {
      if (err?.name === 'AbortError') return false
      toastError(err?.message || '创建失败，请重试')
      return false
    }
  }
  ...
  async function login(payload) {
    try {
      ...
      if (err?.name === 'AbortError') return false
      toastError(err?.message || '登录失败，请重试')
      return false
    }
  }
  ...
  async function resetPassword(payload) {
    try {
      ...
      if (ok) toastSuccess('密码已重置')
      return ok
    } catch (err) {
      if (err?.name === 'AbortError') return false
      toastError(err?.message || '重置失败，请重试')
      return false
    }
  }
