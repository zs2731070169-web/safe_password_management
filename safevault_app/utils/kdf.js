 ...
 *
 * 依赖跨端 crypto 句柄 `webcrypto`（App = noble 垫片 / H5 = 原生 WebCrypto），与 services/crypto.js 同源。
 * **不再裸写全局 `crypto`**——真机基座预置的 `crypto` 全局无法被垫片覆盖，裸用会取到残缺 subtle
 * （详见 utils/cryptoPolyfill.js 顶部说明）。
 */
