...
严格按时序图 §6 步骤推进（步骤号与图中 autonumber 对应；限流由路由层先行处理，此处不重复）：
  ...
  7. 决策点 C → 走 C1（接受丢失，纯零知识）：返回 { resetOk: true, cloudBackupCleared: true }，
     不签发 token、不自动登录。
...
决策点 C（与零知识冲突的取舍）——本实现取 C1：
  重置只换后端登录凭据，**无法解开旧密码包裹的云端 blob**。模块 2（备份服务）尚未实现，故按
  C1「接受丢失」处理：直接把旧 blob 视为不可解密、标记失效（cloudBackupCleared=true），由客户端
  提示用户「云备份需重新上传」。待模块 2 接入后，可改走 C2（恢复凭据 key escrow）：客户端另用
  恢复码 GET /backup/recovery-blob 解出 DataKey，再以新密码重新包裹，届时本编排改返回 recoverable。
...
async def reset_account(
    ...
    """忘记密码重置主流程，返回 { resetOk: True, cloudBackupCleared: True }（走决策点 C1）。
    ...
    :param code: 邮箱验证码
    :returns: {"resetOk": True, "cloudBackupCleared": True}
    ...
    # 步骤 7：决策点 C → C1（接受丢失，纯零知识）。旧密码包裹的云端 blob 无法解开，标记失效、
    # 提示用户重新上传；不签发 token、不自动登录。待模块 2 接入后可改走 C2 用恢复凭据恢复 DataKey。
    return {"resetOk": True, "cloudBackupCleared": True}
