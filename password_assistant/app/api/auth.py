...
from services.rate_limit import enforce_ip_limit, enforce_send_code_limit
...
def rate_limit(scope: str):
    """IP 维度限流依赖工厂（DI 适配层）：把「取 IP → 限流」收敛成可挂在路由上的依赖。

    纯 IP 限流只依赖 Request，故用 dependencies=[Depends(rate_limit(scope))] 挂载，
    在解析请求体 / 连库之前即先行拦截，且不污染端点函数签名；阈值与 key 前缀由
    service 层的 scope 表决定。依赖体依赖请求体的场景（如发码按邮箱）不走此工厂。
    """
    async def _dependency(request: Request) -> None:
        await enforce_ip_limit(scope, _client_ip(request))
...
async def reset_password(
    ...
    流程对齐时序图 §6：
      ...
      7) 决策点 C → C1（接受丢失，纯零知识）：返回 200 { resetOk: true, cloudBackupCleared: true }

    与 §5 改密的差异：重置发生在「忘记密码、无法登录」场景，**无 access token**，仅凭验证码授权，
    故请求体携带 email；且重置**不签发 token、不自动登录**（对齐图末「回登录页重新登录」）——前端
    在重置成功后随即用新密码走一次 §3 登录拿合法会话。决策点 C 取 C1：旧密码包裹的云端 blob
    无法解开，标记失效、提示用户重新上传；待模块 2 接入后可改走 C2 用恢复凭据恢复 DataKey。
    """
