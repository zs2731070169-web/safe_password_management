...
SafeVault 是一款**零知识架构**的移动端密码管理应用 —— 服务端永不接触明文密码与库数据，你的密码只有你知道。采用端到端加密（AES-256-GCM + PBKDF2/Argon2id），支持云账户认证、加密备份、恢复码体系、密码健康检测与生物识别解锁。

> 🎯 **关于本项目**：SafeVault 是一款从 **0 → 1** 使用 **Vibe Coding** 全链路闭环的学习与交流项目，按照商业项目的完整生命周期构建——从产品需求、交互设计、系统设计、开发实现、测试验证、部署运维、运营到推广，全流程均可作为参考。**本项目本身不具备任何商业性质**，所有加密算法均为开源实现，欢迎学习、讨论与改进。
