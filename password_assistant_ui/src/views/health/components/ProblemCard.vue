...
<template>
  <article class="problem-card" :class="`problem-card--${entry.severity}`">
    <!-- 图标圆底（严重度分色） -->
    <div class="problem-card__badge" aria-hidden="true">
    ...
    <div class="problem-card__body">
      ...
      <div class="problem-card__top">
        <div class="problem-card__meta">
          <h3 class="problem-card__name">{{ entry.name }}</h3>
          <p class="problem-card__type">{{ entry.type }}</p>
        </div>
        <span class="problem-card__tag">{{ entry.tag }}</span>
      </div>
