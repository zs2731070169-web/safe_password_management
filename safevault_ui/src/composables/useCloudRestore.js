 ...
 *       · ok           → 调 vault.replaceFromSnapshot 覆盖本地库 + ElMessage.success('已从云端恢复')
    ...
    try {
      ...
      switch (res.status) {
        ...
        case 'undecryptable':
          ...
          ElMessage.warning('云备份不可同步(由于重置过密码)，请重新备份')
          break
        case 'skipped':
          ElMessage.warning('请先登录后再从云端同步')
          break
      ...
      ElMessage.error(err?.message || '云端恢复失败，请稍后重试')
    } finally {
