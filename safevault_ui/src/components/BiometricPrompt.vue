...
<template>
  <Teleport to="body">
    <Transition name="bp-modal">
      <div v-if="visible" class="bp-modal" @click.self="cancel">
        <div
        ...
        >
          <h3 id="bp-modal-title" class="bp-modal__title">{{ copy.title }}</h3>
          <p class="bp-modal__hint">{{ copy.hint }}</p>
          ...
          <button
            ...
            @click="startScan"
          >
            <span class="bp-bio__ring">
              <AppIcon name="fingerprint" :width="40" :height="44" />
            </span>
            <span class="bp-bio__label">{{ tapLabel }}</span>
          </button>

          <p v-if="errorMsg" class="bp-modal__error">{{ errorMsg }}</p>
