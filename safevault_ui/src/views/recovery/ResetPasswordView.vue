<script setup>
/**
 * ResetPasswordView —— 忘记密码：邮箱验证码重置账户密码 + 恢复码恢复数据（两步）
 *
 * 统一身份后找回改为邮箱验证码重置。用户从登录页「忘记密码？」进入：
 *   步骤一（reset）：填邮箱（预填已记住账户）→ 发送验证码 → 输验证码 + 新密码 → 重置成功。
 *   步骤二（recover）：重置成功后（store.pendingRecovery 为 true），此前数据用旧密码包裹的
 *     DataKey 已作废，需用「恢复码」取回 DataKey 才能找回数据；输入恢复码恢复（或跳过，
 *     数据为空也可进库，符合零知识——用户可日后再恢复）。
 *
...
/** 恢复 / 跳过完成：进入密码库（RecoverDataStep 内部已处理恢复成功 / 跳过的反馈） */
