 ...
 *
 * 仅依赖标准 WebCrypto（window.crypto.subtle），与 utils/kdf.js 同源，Capacitor WebView 与现代浏览器均支持。
 */
...
export async function encryptJson(dataKey, obj) {
  ...
  const cipherBuf = await crypto.subtle.encrypt({ name: 'AES-GCM', iv }, dataKey, plaintext)
  // 拼装 IV ‖ 密文+Tag
...
export async function decryptJson(dataKey, ciphertext) {
  ...
  const plainBuf = await crypto.subtle.decrypt({ name: 'AES-GCM', iv }, dataKey, cipher)
...
export async function wrapDataKey(kek, dataKeyRaw) {
  const iv = crypto.getRandomValues(new Uint8Array(IV_LENGTH))
  const cipherBuf = await crypto.subtle.encrypt({ name: 'AES-GCM', iv }, kek, dataKeyRaw)
  const payload = new Uint8Array(iv.byteLength + cipherBuf.byteLength)
  payload.set(iv, 0)
  payload.set(new Uint8Array(cipherBuf), iv.byteLength)
...
export async function unwrapDataKeyRaw(kek, wrapped) {
  ...
  const iv = payload.slice(0, IV_LENGTH)
  const cipher = payload.slice(IV_LENGTH)
  const plainBuf = await crypto.subtle.decrypt({ name: 'AES-GCM', iv }, kek, cipher)
